...
def test_oracle_id_mapping_covers_all_14_exactly_once():
    backend_root = Path(__file__).resolve().parents[2]
    mapping = json.loads(
        (
            backend_root
            / "app"
            / "evaluation_demo"
            / "patient_scenario_map.json"
        ).read_text(encoding="utf-8")
    )

    by_id = mapping["patientsById"]
    assert len(by_id) == 10
    assert set(by_id) == {
        "12724065", "12724066", "12724067", "12724068", "12724069",
        "12724070", "12724071", "12742397", "12742399", "12742400",
    }

    assigned = [
        scenario_id
        for plan in by_id.values()
        for scenario_id in plan.get("scenarioIds", [])
    ]

    assert len(assigned) == 14
    assert len(set(assigned)) == 14
    assert set(assigned) == set(ALL_14)

    dual = [
        patient_id
        for patient_id, plan in by_id.items()
        if len(plan.get("scenarioIds", [])) == 2
    ]
    single = [
        patient_id
        for patient_id, plan in by_id.items()
        if len(plan.get("scenarioIds", [])) == 1
    ]
