...
class SlmWidgetAssembler:
    def assemble(self, *, incident_id: str) -> dict[str, Any]:
        ...
        # V7 evaluation responses are already the model-owned interpretation.
        # Return them directly so the legacy deterministic Phase 6 overlay never
        # re-enters the evaluation path. The phase7 directory name is retained
        # only because the existing frontend endpoint reads this storage location.
        if (
            isinstance(stored_response, dict)
            and stored_response.get("validationMode") == "etiology_v7_contract"
        ):
            model_payload, model_state = parse_model_payload(stored_response)
            widget = _dict(model_payload.get("widgetInterpretation"))
            if widget:
                explicit_model_state = _dict(model_payload.get("modelState"))
                return {
                    "schemaVersion": "slm-widget-v7-compat-v1",
                    ...
                    "scenarioId": model_payload.get("scenarioId"),
                    "widgetInterpretation": widget,
                    # Stable V7 frontend contract. Keep modelResponse for audit/
                    # compatibility, but UI code should prefer clinicalInterpretation.
                    "clinicalInterpretation": model_payload.get("modelResponse"),
                    "modelResponse": model_payload.get("modelResponse"),
                    "displayModelResponse": model_payload.get("displayModelResponse"),
                    "responseProvenanceLabel": model_payload.get("responseProvenanceLabel"),
                    "precomputedResponse": model_payload.get("precomputedResponse"),
                    "responseMeta": model_payload.get("responseMeta") or {},
                    "modelState": explicit_model_state or model_state,
                    "validationStatus": (
                        stored_response.get("validationStatus")
                        or "contract_valid"
                    ),
                    "provenance": {
                        ...
                        "inferenceMode": (
                            "precomputed"
                            if (explicit_model_state or model_state).get("precomputed")
                            else "live"
                        ),
                        "phase6Used": False,
