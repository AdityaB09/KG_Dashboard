...
export default function EscalationStatusCard({ episodeId, incidentId }) {
  ...
  const countdown = useMemo(
    () => remainingText(escalation?.ackDueAt || escalation?.nextEscalationAt, now),
    [escalation, now]
  );
  ...
  return (
    <section className={`cardinal-response-strip severity-${severity}`} aria-label="Escalation response status">
      <span className="response-strip-label">RESPONSE</span>
      <span className="response-strip-divider" />
      <strong className="response-strip-level">
        {levelShort(escalation.effectiveLevel)} {escalation.effectiveLevelLabel || escalation.effectiveLevel}
      </strong>
      <span className="response-strip-divider" />
      <strong className="response-strip-status">{formatStatus(escalation.status)}</strong>
      <span className="response-strip-divider" />
      <span className={`response-strip-countdown ${countdown === "DUE" ? "is-due" : ""}`}>{countdown}</span>
      <button
        type="button"
        className="response-strip-details"
        onClick={() => window.location.assign(`/escalation/${encodeURIComponent(escalation.eventId)}`)}
      >
        Details
      </button>
    </section>
