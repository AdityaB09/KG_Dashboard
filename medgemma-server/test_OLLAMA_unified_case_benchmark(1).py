...
UNIFIED_CASE_SIMULATION_CONTROL_MESSAGE = {
    "role": "system",
    "content": f"""
This run is an offline synthetic unified-case benchmark for comparing local Ollama models.

Treat the supplied deterministic ECG evidence and supplied clinical context as one unified hypothetical patient case for this benchmark. Do not discuss cross-dataset linkage, provenance mismatch, separate source identities, or whether the records came from different people. Do not mention the benchmark transformation in the clinical JSON response.
...
Grounding requirements:
1. Use only the supplied values.
2. Consolidate duplicate records and avoid repeating the same sentence.
3. Distinguish deterministic ECG measurements from reference annotations.
4. Distinguish episode-near clinical context from historical or remote context.
5. Do not infer causation from temporal association.
6. MedicationRequest records are orders, not proof that medication was taken.
7. Dataset V annotations are reference evidence and not an independent diagnosis.
8. A zero independent morphology-candidate count does not negate V annotations.
9. Do not invent interpretations for numeric metrics when the package does not define them.
10. Return JSON only. Do not use Markdown or code fences.
11. Keep the response complete and reasonably concise.
12. Put a research/non-diagnosis disclaimer in safetyStatement. Preferred wording:
    {REQUIRED_SAFETY_SENTENCE}
""".strip(),
}
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--unified-case-simulation",
        action="store_true",
        help=(
            "Offline benchmark mode. The model-facing package is transformed "
            "into one synthetic unified patient case and source-linkage mismatch "
            "language is removed. The saved wrapper remains marked as synthetic."
        ),
    )
...
def contains_case_linkage_language(value: Any) -> bool:
    text_value = str(value).lower()
    return any(
        phrase in text_value
        for phrase in _CASE_LINKAGE_PHRASES
    )


def sanitize_instruction_text_for_unified_case(content: str) -> str:
    replacements = {
        (
            "Do not claim that historical Oracle sandbox data caused an INCART ECG event. "
            "Do not claim that the INCART waveform and Oracle FHIR data came from the same real patient."
        ): (
            "For this offline synthetic unified-case benchmark, treat the supplied ECG "
            "and clinical context as one hypothetical patient case. Do not discuss "
            "cross-source linkage or dataset provenance. Do not infer causation."
        ),
        (
            "Do not claim that the INCART ECG and Oracle/FHIR records are from the same real patient."
        ): (
            "Treat the supplied ECG and clinical records as one hypothetical patient "
            "case for this offline benchmark. Do not discuss source linkage."
        ),
        (
            "Do not claim that the INCART waveform and Oracle FHIR data came from the same real patient."
        ): (
    ...
    # Remove remaining source-linkage bullets that may vary slightly.
    cleaned_lines: list[str] = []
    for line in result.splitlines():
        if contains_case_linkage_language(line):
            continue
...
def sanitize_evidence_for_unified_case(value: Any) -> Any:
    """
    Create a model-facing synthetic unified-case package.

    This does not alter the original input file. It removes only cross-source
    linkage/provenance statements that would make the model discuss the
    ECG/FHIR pairing. Clinical values, measurement uncertainty, timing,
    medication evidence level, and non-diagnosis controls are preserved.
    """
    if isinstance(value, dict):
        ...
        for key, child in value.items():
            ...
            # Remove model-facing linkage fields. The script's saved wrapper
            # records that this was a synthetic unified-case benchmark.
            ...
            if normalized_key == "sourceresolution":
                output[key] = {
                    "source": "stored_clinical_context",
                    "status": "ready",
                }
                continue

            if normalized_key == "provenance":
                sanitized_provenance = sanitize_evidence_for_unified_case(
                    child
                )

                if isinstance(sanitized_provenance, dict):
                    sanitized_provenance.pop(
                        "clinicalContextSource",
                        None,
                    )
                    sanitized_provenance.pop(
                    ...
                    sanitized_provenance["caseMode"] = (
                        "synthetic_unified_case_benchmark"
                    )

                output[key] = sanitized_provenance
                continue

            if normalized_key == "requiredoutputbehavior":
                items = [
                    item
                    for item in (
                        child
                    ...
                    if not contains_case_linkage_language(item)
                ]

                items.append(
                    "Treat the supplied evidence as one unified hypothetical "
                    "patient case for this offline benchmark."
                )
                output[key] = items
                continue

            if normalized_key == "limitations":
                output[key] = [
                    item
                    for item in (
                        child
            ...
            output[key] = sanitize_evidence_for_unified_case(
                child
            )

        if (
            output.get("safety")
            and isinstance(output["safety"], dict)
        ):
            output["safety"]["caseMode"] = (
                "synthetic_unified_case_benchmark"
            )
    ...
    if isinstance(value, list):
        return [
            ...
            if not (
                isinstance(item, str)
                and contains_case_linkage_language(item)
            )
        ]

    if isinstance(value, str):
        return sanitize_instruction_text_for_unified_case(
            value
        )
...
def sanitize_user_message_for_unified_case(content: str) -> str:
    ...
    instruction_text, json_text = content.split(
        marker,
        1,
    )

    instruction_text = (
        sanitize_instruction_text_for_unified_case(
            instruction_text
        )
    )
    ...
    try:
        ...
        # Keep the message usable even when the evidence JSON is formatted
        # unexpectedly. Instruction sanitization still removes the mismatch
        # language.
        return (
            f"{instruction_text}\n\n"
            f"{marker}\n"
    ...
    sanitized_evidence = (
        sanitize_evidence_for_unified_case(
            evidence
        )
    )
    ...
    return (
        f"{instruction_text}\n\n"
        f"{marker}\n"
        f"{json.dumps(sanitized_evidence, ensure_ascii=False, separators=(',', ':'))}"
    )


def prepare_unified_case_messages(
    ...
    for message in messages:
        ...
        if role == "user":
            content = sanitize_user_message_for_unified_case(
                content
            )
        else:
            content = sanitize_instruction_text_for_unified_case(
                content
            )
        ...
        if not content.strip():
            if role == "system":
                content = (
                    ...
                    "Do not invent clinical facts or measurements."
                )
            else:
                continue
...
def build_messages(
    ...
    unified_case_simulation: bool = False,
) -> list[dict[str, str]]:
    messages: list[dict[str, str]] = []

    control_message = (
        UNIFIED_CASE_SIMULATION_CONTROL_MESSAGE
        if unified_case_simulation
        else OUTPUT_CONTROL_MESSAGE
    )
...
def run_until_usable(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        messages = build_messages(
            ...
            unified_case_simulation=unified_case_simulation,
        )
...
def main() -> None:
    ...
    default_suffix = (
        "_SYNTHETIC_UNIFIED_CASE"
        if args.unified_case_simulation
        else ""
    )
    ...
    output_file = (
        ...
        / f"SLM_Response_{output_key}{default_suffix}.json"
    )
    ...
    print(
        "Case mode: "
        + (
            "synthetic unified-case benchmark"
            if args.unified_case_simulation
            else "source-separated research mode"
        )
    )
    print()
    ...
    try:
        ...
        if args.unified_case_simulation:
            package_messages = (
                prepare_unified_case_messages(
                    package_messages
                )
            )

            print(
                "Synthetic unified-case transformation is active. "
                "The original input file is not modified."
            )
            print(
                "Do not use this benchmark output as real patient evidence."
            )
        ...
        (
            ...
            unified_case_simulation=(
                args.unified_case_simulation
            ),
        )
        ...
        saved_result = {
            ...
            "caseMode": (
                "synthetic_unified_case_benchmark"
                if args.unified_case_simulation
                else "source_separated_research"
            ),
            "notForClinicalUse": bool(
                args.unified_case_simulation
            ),
