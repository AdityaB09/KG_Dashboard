...
export default function EscalationPage({ eventId }) {
  ...
  const terminalPathway = escalation?.effectiveLevel === "CODE_RESPONSE_ACTIVATION";
  ...
  async function toggleAutoEscalation() {
    if (!escalation || terminalPathway || action) return;
    await runAction(
  ...
  return (
    <main className="cardinal-escalation-page">
      <header className="cardinal-escalation-header">
        <div><span className="cardinal-escalation-kicker">CARDINAL · Clinical Response Routing</span><h1>{valueOrDash(escalation.effectiveLevelLabel)}</h1><p>{valueOrDash(escalation.assignedRole)} · Site-configurable hospital response pathway</p></div>
      ...
      <section className="cardinal-escalation-grid">
        <article className="cardinal-escalation-card">
          ...
          <div className="cardinal-escalation-fields">
            ...
            <Field label="Automatic Escalation">{terminalPathway ? "Not applicable (terminal response)" : autoEscalationEnabled ? "ON" : "OFF"}</Field>
      ...
      <section className="cardinal-escalation-card cardinal-escalation-actions-card">
        ...
        <div className="cardinal-escalation-actions cardinal-routing-controls">
          ...
          <div className={`cardinal-auto-toggle${autoEscalationEnabled ? " is-on" : ""}${terminalPathway ? " is-disabled" : ""}`}>
            <span>Automatic Escalation</span>
            <button
              type="button"
              role="switch"
              aria-checked={autoEscalationEnabled}
              disabled={terminalPathway || Boolean(action)}
              onClick={toggleAutoEscalation}
            >
              <span className="cardinal-auto-toggle-track"><span className="cardinal-auto-toggle-knob" /></span>
              <strong>{terminalPathway ? "N/A" : autoEscalationEnabled ? "ON" : "OFF"}</strong>
            </button>
          </div>
        </div>
      </section>
