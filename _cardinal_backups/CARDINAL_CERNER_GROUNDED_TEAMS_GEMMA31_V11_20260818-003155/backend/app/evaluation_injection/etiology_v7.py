...
- Recommend ONE clinical response pathway from the site-configurable hospital response
  profile below. This is a recommendation only; CARDINAL's deterministic site policy
  will independently adjudicate it. Oracle Health Millennium itself does NOT define a
  universal numbered L1-L4 escalation ladder:
    MONITOR_ONLY
    CARE_TEAM_REVIEW
    URGENT_PROVIDER_REVIEW
    RAPID_RESPONSE_ACTIVATION
    CODE_RESPONSE_ACTIVATION
- Use CODE_RESPONSE_ACTIVATION only for cardiac/respiratory arrest, pulselessness, or
  immediately life-threatening instability requiring bedside resuscitation/code-team
  activation. Use RAPID_RESPONSE_ACTIVATION for serious acute deterioration requiring
  an urgent multidisciplinary response team when the patient is not in arrest.
...
def validate_v7_response(
    ...
    *,
    ...
    # Accept both V10 canonical pathway codes and legacy L0-L4 values so old
    # precomputed evidence remains usable. Always normalize to the V10 code.
...
def _widget_interpretation(
    *,
    ...
    return {
        "schemaVersion": "cardinal-etiology-widget-interpretation-v7.1",
        "severity": episode.get("severity") or "warning",
        "statusLabel": (
            "Precomputed SLM interpretation"
            if metadata.get("precomputed")
            else "SLM interpretation"
        ),
        "displayPolicy": "always_show_model_response",
        # The model-owned rhythm is the headline. Do not use an upstream
        # scenario/rhythm label as the primary display title.
        "headline": response.get("rhythm") or "Evaluation episode",
        ...
        # Native V7 fields. The frontend should prefer these instead of
        # reconstructing the new response from legacy Phase-6-era DTO keys.
        "episodeSummary": response.get("episodeSummary") or "",
        "rhythm": response.get("rhythm") or "",
        "primaryEtiology": response.get("primaryEtiology") or "",
        "mechanism": response.get("mechanism") or "",
        ...
        "uncertainty": list(response.get("uncertainty") or []),
...
def _store_precomputed_unavailable(
    *,
    record: dict[str, Any],
    scenario_id: str,
    episode_id: str,
    ...
    """Keep the capture/UI alive when a stored model artifact is invalid.

    One supplied V7 model/scenario result (Gemma 4 E4B / FLUTTER-IC-011) is not
    contract-valid. Deployed precomputed mode must never silently substitute a
    different model or fabricate a repaired clinical answer. Instead, persist an
    explicit unavailable response that the frontend can present transparently.
    """
    profile = _precomputed_profile()
    widget = {
        "schemaVersion": "cardinal-etiology-widget-interpretation-v7.1",
        "severity": (record.get("episode") or {}).get("severity") or "warning",
        "statusLabel": "Precomputed response unavailable",
        "displayPolicy": "always_show_model_response",
        "headline": "SLM response unavailable",
        "episodeNarrative": (
            f"No contract-valid precomputed response is available for {profile} "
            f"on {scenario_id}."
        ),
        ...
        "recommendedActions": [],
        "uncertainty": [error],
        "keyMetrics": _widget_metrics(record, {}),
        "possibleContributors": [],
        "importantLimitations": [error],
        "materialEtiologicUncertainty": [error],
        "validationSummary": {
            "status": "precomputed_unavailable",
            "strictlyAccepted": False,
            "displayableWithReview": True,
        ...
        "evaluationStatistics": {
            ...
            "safetyPass": None,
            "attemptCount": None,
            "generationLatencySeconds": None,
            "rawResponseDisplayed": False,
            "validatedResponseAvailable": False,
        },
    }
    ...
    stored_response = {
        ...
        "modelAlias": profile,
        "model": profile,
        "validationStatus": "precomputed_unavailable",
        # Keep the V7 direct assembler path; this is a V7 response-state record,
        # not a request to fall back to the legacy deterministic overlay.
        ...
        "response": {
            ...
            "responseProvenanceLabel": f"Precomputed {profile} response unavailable",
            "precomputedResponse": {
                "profile": profile,
                "scenarioId": scenario_id,
    ...
    _atomic_json(
        ...
        {
            "schemaVersion": "etiology-v7-status-compat-v1",
            "state": "unavailable",
            "status": "unavailable",
            "incidentId": incident_id,
            "episodeId": episode_id,
    ...
    result = {
        "status": "unavailable",
        "source": "precomputed_v7_unavailable",
        "model": profile,
        "modelResponse": None,
        "displayModelResponse": None,
        "widgetInterpretation": widget,
    ...
    _atomic_json(
        episode_dir / "etiology_v7_precomputed_unavailable.json",
        {
            "schemaVersion": "cardinal-etiology-v7-unavailable-v1",
            "createdAt": _now_iso(),
            "scenarioId": scenario_id,
            "episodeId": episode_id,
            "incidentId": incident_id,
...
async def run_etiology_v7(
    *,
    ...
    if _precomputed_enabled():
        ...
        except EtiologyV7Error as exc:
            if _precomputed_required():
                return _store_precomputed_unavailable(
                    record=record,
    ...
    stored_response = {
        ...
        "response": {
            ...
            "responseMeta": {
                ...
                "profile": metadata.get("profile"),
                "runNumber": metadata.get("runNumber"),
