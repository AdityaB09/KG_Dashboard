...
def _next_response_window_iso(level: EscalationLevel, enabled: bool) -> str | None:
    if not enabled or is_terminal_level(level) or level == EscalationLevel.MONITOR_ONLY:
        return None
...
class EscalationOrchestrator:
    ...
    async def evaluate_and_dispatch(
        ...
        *,
        ...
        append_audit_event(
            ...
            data={"modelSuggestedLevel": model_level.value, "confidence": confidence},
        )
        append_audit_event(
            event_id,
            "POLICY_EVALUATED",
        ...
        append_audit_event(
            ...
            data={"responsePathway": effective.value, "assignedRole": level_role(effective)},
            new_state="CREATED",
        )
        append_audit_event(
            event_id,
            ...
            data={"responsePathway": effective.value},
        )
    ...
    async def _dispatch_level(self, event_id: str, level: EscalationLevel, *, reason: str) -> dict[str, Any]:
        ...
        def mark_started(case: dict[str, Any]) -> None:
            ...
            if not history or normalize_level(history[-1].get("level"), default=EscalationLevel.MONITOR_ONLY) != level:
                history.append({"level": level.value, "label": level_label(level), "enteredAt": started_at, "reason": reason})
        ...
        append_audit_event(
            ...
            data={"responsePathway": level.value, "assignedRole": level_role(level), "reason": reason},
        ...
        append_audit_event(
            ...
            data={"responsePathway": level.value, "provider": case.get("provider")},
        ...
        auto_enabled = bool(current.get("autoEscalationEnabled", settings.ESCALATION_AUTO_ADVANCE_DEFAULT))
        ...
        append_audit_event(
            ...
            detail=(
                ...
                if next_at
                else ("Terminal response pathway; automatic escalation does not advance further." if is_terminal_level(level) else "Automatic escalation is OFF for this case.")
            ),
            data={"enabled": auto_enabled, "nextEscalationAt": next_at, "responsePathway": level.value},
        )
    ...
    def set_auto_escalation(self, event_id: str, *, enabled: bool) -> dict[str, Any]:
        ...
        effective_enabled = bool(enabled) and not is_terminal_level(current) and current != EscalationLevel.MONITOR_ONLY
        ...
        append_audit_event(
            ...
            data={"enabled": effective_enabled, "nextEscalationAt": next_at, "responsePathway": current.value},
        )
    ...
    async def process_due_timeouts(self) -> list[str]:
        ...
        for case in escalation_repository.due_cases(now_iso()):
            ...
            append_audit_event(
                ...
                data={"responsePathway": current.value},
            )
            escalation_repository.update(event_id, lambda value: value.update({"nextEscalationAt": None}))
