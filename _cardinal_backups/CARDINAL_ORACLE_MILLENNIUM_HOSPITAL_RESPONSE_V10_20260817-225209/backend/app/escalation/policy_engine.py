...
class EscalationPolicyEngine:
    ...
    def load_policy(self) -> tuple[dict[str, Any], str]:
        ...
        try:
            ...
            fail_safe = normalize_level(
                getattr(settings, "ESCALATION_POLICY_FAIL_SAFE_LEVEL", "L2_URGENT_PROVIDER_REVIEW"),
                default=EscalationLevel.L2_URGENT_PROVIDER_REVIEW,
            )
            return {
                "policyId": "CARDINAL-FAIL-SAFE",
                "version": "1",
                "defaultMinimumLevel": fail_safe.value,
                "failSafeLevel": fail_safe.value,
                "scenarioMinimums": {},
                "evidenceRules": [],
                "ackTimeoutSecondsByLevel": {},
            }, "fail_safe"
    ...
    def evaluate(
        ...
        *,
        ...
        default_level = normalize_level(
            ...
            or getattr(settings, "ESCALATION_POLICY_DEFAULT_LEVEL", "L1_NURSING_REVIEW"),
            default=EscalationLevel.L1_NURSING_REVIEW,
        )
        ...
        evidence_level = EscalationLevel.L0_MONITOR
        ...
        for rule in policy.get("evidenceRules") or []:
            ...
            if not matched:
                continue
            level = normalize_level(rule.get("minimumLevel"), default=EscalationLevel.L0_MONITOR)
        ...
        return PolicyDecision(
            level=minimum,
            policy_id=str(policy.get("policyId") or getattr(settings, "ESCALATION_POLICY_ID", "CARDINAL-HOSPITAL-V1")),
    ...
    def ack_timeout_seconds(self, level: EscalationLevel | str) -> float:
        ...
        override_raw = os.getenv("ESCALATION_ACK_TIMEOUTS_JSON", "").strip()
        if override_raw:
            try:
                override = json.loads(override_raw)
            except json.JSONDecodeError:
                override = {}
            if isinstance(override, dict) and normalized.value in override:
                try:
                    return max(0.0, float(override[normalized.value]))
                except (TypeError, ValueError):
                    pass
        policy, _ = self.load_policy()
        configured = policy.get("ackTimeoutSecondsByLevel") or {}
        if isinstance(configured, dict) and normalized.value in configured:
            try:
                return max(0.0, float(configured[normalized.value]))
            except (TypeError, ValueError):
                pass
        return max(0.0, float(getattr(settings, "ESCALATION_ACK_TIMEOUT_SECONDS", 120)))
    ...
    def public_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "ackTimeoutSecondsByLevel": policy.get("ackTimeoutSecondsByLevel") or {},
