...
def _resolve_gcloud_invocation() -> tuple[list[str], str]:
    ...
    if suffix in {".cmd", ".bat"}:
        ...
        # /d disables AutoRun; /s keeps quoting rules predictable; /c executes then exits.
        command_line = f'"{existing}" auth print-identity-token'
        return [comspec, "/d", "/s", "/c", command_line], str(existing)
