...
REQUIRED = [
 ...
 'infra/scripts/00_CONFIGURE_PROJECT.ps1','infra/scripts/FIRST_TIME_SETUP.ps1','infra/scripts/06_CONFIGURE_GITHUB_VARIABLES.ps1','infra/scripts/DESTROY_APP.ps1','infra/scripts/MODEL_ON.ps1','infra/scripts/MODEL_OFF.ps1',
...
def main():
 ...
 print('=== CARDINAL TERRAFORM + CI/CD V2 PROJECT-AGNOSTIC VERIFIER ===')
 ...
 # Project portability guard: Terraform and GitHub workflow files must not embed
 # a concrete kg-dashboard project ID or a 12-digit GCP project number.
 scan_roots=[root/'infra', root/'.github']
 hits=[]
 for scan_base in scan_roots:
  if scan_base.exists():
   for path in scan_base.rglob('*'):
    if not path.is_file() or path.suffix.lower() not in {'.tf','.yml','.yaml'}: continue
    txt=path.read_text(encoding='utf-8',errors='ignore')
    if re.search(r'kg-dashboard-\d{4,}',txt) or re.search(r'(?<!\d)\d{12}(?!\d)',txt):
     hits.append(str(path.relative_to(root)))
 check(not hits, f'Terraform/workflows contain no hardcoded GCP project identity (found={hits})')
 common=(root/'infra/scripts/Common.ps1').read_text(encoding='utf-8',errors='replace') if (root/'infra/scripts/Common.ps1').exists() else ''
 check('gcloud projects describe $id' in common and 'ProjectNumber = $number' in common,'Project number is discovered from GCP at runtime')
 deploy=(root/'.github/workflows/deploy.yml').read_text(encoding='utf-8',errors='replace') if (root/'.github/workflows/deploy.yml').exists() else ''
 check('${{ vars.GCP_PROJECT_ID }}' in deploy and '${{ vars.GCP_PROJECT_NUMBER }}' in deploy,'GitHub deploy reads project identity from repository variables')
