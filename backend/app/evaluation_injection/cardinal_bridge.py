...
def _final_cardinal_response(
    *,
    ...
    source = str((diagnostic_event.get("source") or {}).get("name") or "upstream source")

    return {
        "episodeSummary": str(model_response.get("episodeSummary") or "").strip(),
        "rhythmInterpretation": (
            f"Authoritative upstream diagnosis: {diagnosis}. Source: {source}. "
            "The rhythm diagnosis was not generated by the SLM."
        ),
        "clinicalContext": str(model_response.get("detectedEpisodeContext") or "").strip(),
        "mostLikelyEtiology": str(model_response.get("mostLikelyEtiology") or "").strip(),
        "contributingFactors": [
            str(item).strip()
            for item in model_response.get("contributingFactors") or []
            if str(item).strip()
        ],
        # Compatibility field only. It is not model-owned or scored.
        "recommendedImmediateActions": [],
        "uncertaintyAndMissingData": [
            str(item).strip()
            for item in model_response.get("uncertaintyAndMissingData") or []
            if str(item).strip()
        ],
    }
...
def _validation_hard_pass(validation: dict[str, Any]) -> bool:
    ...
    status = str(validation.get("groundingStatus") or validation.get("status") or "unknown")
    return bool(
        status in {"accepted", "accepted_with_review"}
        and not validation.get("hardErrors")
        and not validation.get("contradictions")
...
def _widget_interpretation(
    *,
    ...
    """Always show the latest generated SLM response; evaluate it separately."""
    diagnosis = str((diagnostic_event.get("diagnosis") or {}).get("display") or "")
    return {
        "headline": diagnosis,
        "statusLabel": "SLM response generated",
        "displayPolicy": "always_show_model_response",
        "episodeNarrative": cardinal["episodeSummary"],
        "arrhythmiaNarrative": cardinal["rhythmInterpretation"],
        "morphologyNarrative": "",
        "currentSituationNarrative": cardinal["clinicalContext"],
        "rootCauseNarrative": cardinal["mostLikelyEtiology"],
        "possibleContributors": cardinal["contributingFactors"],
        "importantFindings": cardinal["contributingFactors"],
        "importantLimitations": cardinal["uncertaintyAndMissingData"],
    ...
    *,
    ...
    return {
        ...
        "narrative": {
            ...
            "clinicalContext": cardinal.get("clinicalContext"),
            "mostLikelyEtiology": cardinal.get("mostLikelyEtiology"),
            "contributingFactors": cardinal.get("contributingFactors") or [],
            "uncertaintyAndMissingData": cardinal.get("uncertaintyAndMissingData") or [],
        },
        ...
        "measurementConflicts": evidence_bundle.get("measurementConflicts") or [],
        "sourceManifest": evidence_bundle.get("sourceManifest") or {},
...
def _model_owned_from_cardinal(
    ...
    """Convert a stored display-cardinal record back to the five model fields."""
    return {
        "episodeSummary": str(cardinal.get("episodeSummary") or "").strip(),
        "detectedEpisodeContext": str(
            cardinal.get("clinicalContext")
            or cardinal.get("detectedEpisodeContext")
            or ""
        ).strip(),
        "mostLikelyEtiology": str(
            cardinal.get("mostLikelyEtiology") or ""
        ).strip(),
        "contributingFactors": [
            str(item).strip()
            for item in cardinal.get("contributingFactors") or []
            if str(item).strip()
        ][:5],
        "uncertaintyAndMissingData": [
            str(item).strip()
            for item in cardinal.get("uncertaintyAndMissingData") or []
            if str(item).strip()
        ],
    }
...
async def _run_precomputed_demo_pipeline(
    *,
    ...
    except PrecomputedResponseError:
    ...
    provenance = {
        ...
        "sourceArtifactSet": (
            "medgemma-27b-it-all8-incart-v6-0-1"
        ),
