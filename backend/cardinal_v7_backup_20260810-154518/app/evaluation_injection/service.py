...
from app.evaluation_injection.cardinal_bridge import (
    build_score_and_attach_cardinal,
)
...
class EvaluationInjectionService:
        ...
        try:
            ...
            phase7_result = await (
                ...
                .run_incident(
                    ...
                    force=False,
                    force_context=False,
                    # Phase 7 contributes deterministic ECG evidence only.
                    # Its clinical context branch reuses the already persisted
                    # complete episode package and never resolves Oracle FHIR.
                    run_model=False,
                    trigger_episode_id=(
                        session.episode_id
                    ),
            ...
            # Do not derive or attach an Oracle patient context. The complete
            # scenario package below is the sole clinical context source.
            ...
            capture_evidence = {
                ...
                "demoRunId": (
                    (metadata.get("oracleDemo") or {}).get("demoRunId")
                ),
                "sampleRateHz": (
                    metadata.get("sampleRate")
                    or metadata.get("sourceSampleRate")
                ),
                "leads": (
                    metadata.get("leadNames")
                    or metadata.get("leadIds")
                    or []
                ),
                ...
                "triggerMode": metadata.get("triggerMode"),
                "detectorRateBpm": (
                    metadata.get(
                        "detectorRateBpm"
                    )
            ...
            score = cardinal_result["score"]

            phase7_result["slmResponse"] = (
                cardinal_result[
                    "phase7SlmResponse"
                ]
            )
            ...
            current_metadata[
                ...
                "status": cardinal_result[
                    "status"
                ],
                "source": cardinal_result[
                    "source"
                ],
                "responseFile": cardinal_result[
                    "responseFile"
                ],
                "model": cardinal_result[
                    "model"
                ],
                "score": score.get("total"),
                "safetyPass": score.get(
                    "safetyPass"
                ),
                "overallPass": score.get(
                    "overallPass"
                ),
                "validation": (
                    cardinal_result[
    ...
    def _persist_episode(
        ...
        metadata = {
            ...
            "phase6WindowedAnalysisRequested": (
                str(
                    os.getenv(
