...
$Comparison = @(
    ...
    Where-Object {
        $_ -is [psobject] -and
        $_.PSObject.Properties.Name -contains "Scenario" -and
        $_.PSObject.Properties.Name -contains "StrictlyAccepted"
    } |
    Sort-Object Scenario
)

if ($Comparison.Count -ne $CompletedCount) {
    throw (
        "Expected $CompletedCount structured scenario rows, " +
        "found $($Comparison.Count)."
    )
}
